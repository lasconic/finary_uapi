...
def test_get_bank_account_type_per_name(
    session: httpx.Client, type: str, name: str, display_name: str
) -> None:
...
def test_get_bank_account_type_per_name_error(session: httpx.Client) -> None:
...
def test_get_cryptocurrency_by_code(
    session: httpx.Client, code: str, name: str
) -> None:
...
def test_get_cryptocurrency_by_code_error(session: httpx.Client) -> None:
...
def test_get_fiatcurrency_by_code(session: httpx.Client, code: str, name: str) -> None:
    fiat = get_fiatcurrency_by_code(session, code)
    assert fiat
    assert fiat["name"] == name


def test_get_fiatcurrency_by_code_error(session: httpx.Client) -> None:
...
def test_get_user_crypto_by_code_error(session: httpx.Client) -> None:
    crypto = get_user_crypto_by_code(
